...
"""
Modified by Francisco Bravo Sanchez July 2021
SincNet code adapted to process NIPS4Bplus calls that are shorter than cw_len

Original SincNet code at https://github.com/mravanelli/SincNet
...
def create_batches_rnd(batch_size,data_folder,wav_lst,N_snt,wlen,fact_amp):
 ...
 for i in range(batch_size):
  ...
  if t_max-t_min > wlen: # random extract if tag is greater than cw_len
    signal_start=np.random.randint(t_min, t_max-wlen)
    signal=signal[signal_start:signal_start+int(wlen)]
  else: #buffer if tag is smaller than cw_len
    if max(0, t_max-wlen) == min(t_min, signal.shape[0]-wlen):
      signal_start = max(0, t_max-wlen)
    else:
      signal_start=np.random.randint(max(0, t_max-wlen), min(t_min, signal.shape[0]-wlen))
    # signal_start=np.random.randint(max(0, t_max-wlen), min(t_min, signal.shape[0]-wlen))
    signal=signal[signal_start : signal_start + wlen]
  # signal = signal[:wlen] # make sure it is not too long and pass to old code using signal
